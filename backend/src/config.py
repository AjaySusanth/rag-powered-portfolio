...
# WHY TWO ROOTS:
# APP_ROOT is always the backend application root — `backend/` on the host,
# `/app` inside Docker. All runtime assets (data/, assets/) are relative to APP_ROOT.
#
# REPO_ROOT is the monorepo root one level above the backend. On the host it is
# `rag-portfolio/`; inside Docker it does not physically exist, but it is only
# used for two things that are host-only: loading .env and locating knowledge/.
# In production, env vars are injected directly and ingestion is run locally,
# so REPO_ROOT paths are never resolved inside the container.
#
# This two-root design makes every path deterministic in both environments without
# conditional logic.
APP_ROOT: Path = Path(__file__).resolve().parent.parent
# backend/   (host)  |  /app   (Docker)

REPO_ROOT: Path = APP_ROOT.parent
# rag-portfolio/   (host)  |  /   (Docker — unused at runtime)

# Backward-compatible alias used by ingestion_service and scripts.
# Points to repo root so `PROJECT_ROOT / "knowledge"` still resolves correctly
# on the host. Inside Docker, ingestion is never triggered at runtime.
PROJECT_ROOT: Path = REPO_ROOT

# Centralized runtime path constants — always correct inside the container.
DATA_DIR: Path = APP_ROOT / "data"
ASSETS_DIR: Path = APP_ROOT / "assets"
RESUME_DIR: Path = ASSETS_DIR / "resume"

# .env file: check backend root first, then repo root (for local dev).
ENV_FILE_PATH = APP_ROOT / ".env" if (APP_ROOT / ".env").exists() else REPO_ROOT / ".env"
...
class Settings(BaseSettings):
    ...
    ENVIRONMENT: str = Field(
        default="development", description="Deployment environment (e.g., development, production)"
    )
    ...
    GRADER_PROVIDER: str = Field(
        default="gemini", description="The provider for the retrieval grader (e.g., gemini, mock)"
    )
    GENERATOR_PROVIDER: str = Field(
        default="gemini", description="The provider for answer generation (e.g., gemini, mock)"
    )
    ...
    ENABLE_QUERY_REWRITER: bool = Field(
        default=False, description="Whether to enable the query rewriter preprocessing step"
    )
    ...
    REWRITER_PROVIDER: str = Field(
        default="gemini", description="The provider for the query rewriter (e.g., gemini, mock)"
    )
    ...
    RATE_LIMIT_REQUESTS: int = Field(default=10, description="Maximum requests allowed per window")
    RATE_LIMIT_WINDOW_SECONDS: int = Field(
        default=60, description="Duration of rate limiting window in seconds"
    )
    ...
    EMBEDDING_CACHE_TTL_SECONDS: int = Field(
        default=604800, description="Time to live for cached embeddings in seconds (default: 7d)"
    )
    ...
    EMBEDDING_MODEL: str = Field(
        default="text-embedding-3-small", description="The model name for embeddings"
    )
    ...
    def validate_production_keys(self) -> "Settings":
        ...
        if self.ENVIRONMENT == "production":
            if "localhost" in self.DATABASE_URL or "127.0.0.1" in self.DATABASE_URL:
                raise ValueError(
                    "DATABASE_URL must not target localhost/127.0.0.1 in production environment."
                )
            if "localhost" in self.REDIS_URL or "127.0.0.1" in self.REDIS_URL:
                raise ValueError(
                    "REDIS_URL must not target localhost/127.0.0.1 in production environment."
                )
        ...
        if self.EMBEDDING_PROVIDER == "azure_openai":
            if not self.AZURE_OPENAI_API_KEY:
                raise ValueError(
                    "AZURE_OPENAI_KEY (or AZURE_OPENAI_API_KEY) must be set when EMBEDDING_PROVIDER is 'azure_openai'"
                )
            if not self.AZURE_OPENAI_ENDPOINT:
                raise ValueError(
                    "AZURE_OPENAI_ENDPOINT must be set when EMBEDDING_PROVIDER is 'azure_openai'"
                )
