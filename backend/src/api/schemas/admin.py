...
class RetrievalTraceRequest(BaseModel):
    """Request payload for triggering a retrieval trace."""

    query: str = Field(..., description="The query to run and trace through the RAG pipeline.")
    session_id: Optional[str] = Field(None, description="Optional tracking identifier.")
...
class IngestSummary(BaseModel):
    ...
    errors: List[str] = Field(
        default_factory=list, description="List of error messages or partial failures encountered."
    )
