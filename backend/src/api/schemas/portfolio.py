...
class StackResponse(BaseModel):
    ...
    cloud: List[str] = Field(
        ..., description="Cloud providers and database/container cloud resources."
    )
    devops: List[str] = Field(
        ..., description="DevOps tools, container orchestration, IaC, CI/CD, and monitoring."
    )
    ai_ml: List[str] = Field(
        ..., description="AI/ML components and techniques (LLM, NLP, vector search)."
    )
...
class HireResponse(BaseModel):
    ...
    currently_available: bool = Field(
        ..., description="Whether the candidate is currently open for offers."
    )
