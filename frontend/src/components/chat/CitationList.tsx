...
export function CitationList({ citations }: CitationListProps) {
  ...
  return (
    <div className="mt-4 pt-3.5 border-t border-border/40 w-full">
      
      {/* Mobile view: Collapsible Accordion */}
      <details className="group md:hidden block outline-none border border-border/30 bg-muted/10 rounded-xl p-2.5">
        ...
        <div className="grid grid-cols-1 gap-2 mt-2.5 transition-all duration-300">
          {citations.map((citation, index) => (
            <CitationItem
              key={`${citation.project}-${citation.layer}-${citation.file}-${index}`}
              citation={citation}
              index={index + 1}
            />
          ))}
      ...
      {/* Desktop view: Persistent Grid layout */}
