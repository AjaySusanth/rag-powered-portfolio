...
async def test_hybrid_retrieve_orchestration(mock_bm25_retrieve, mock_vector_retrieve):
    ...
    # Assert retriever functions called with correct arguments
    mock_vector_retrieve.assert_called_once_with(query="kubernetes", top_k=2, project="n8n")
    mock_bm25_retrieve.assert_called_once_with(query="kubernetes", top_k=2, project="n8n")
